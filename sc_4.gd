...
func _ready() -> void:
	...
	for m in range(nb):
		var a = m*2*PI / nb
		var pos:Vector3 = Vector3(box_size.x/2 - memb_r + memb_r*cos(a) ,box_size.y/2 - memb_r + memb_r*sin(a), 0)
		# Creation d'un element de membrane
		var memb:RigidBody3D = memb_path.instantiate()
		memb.position = pos
		memb.add_to_group("membrane1")
		memb._global_scene = 4
	...
	for m in range(nb):
		var a = m*2*PI / nb
		var pos:Vector3 = Vector3(box_size.x/2 + memb_r*cos(a) ,box_size.y/2 + memb_r - 24 + memb_r*sin(a), 0)
		# Creation d'un element de membrane
		var memb:RigidBody3D = memb_path.instantiate()
		memb.position = pos
